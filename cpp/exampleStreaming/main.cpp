...
class DeviceServiceClient {
  public:
    ...
    void streamingEventLogs (std::string jwt) {
      ...
      StreamDeviceEventLogsResponse response;

      // read ten uplink events before exit
      for (int i = 0; i < 10; i++) {
        if (reader->Read(&response)) {
          std::cout << response.payload_json() << std::endl;
        }
      }
...
//  metadata.set("authorization", insider.GetJWT());
  ...
  DeviceServiceClient monitor(grpc::CreateChannel(
    "0.0.0.0:8080", grpc::InsecureChannelCredentials()));
  monitor.streamingEventLogs (jwt);
